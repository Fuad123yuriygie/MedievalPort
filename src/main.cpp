...
int main() {
    ...
    WindowSystem windowSystem(fileParser); // Create a WindowSystem instance
    GLFWwindow* window = windowSystem.GetWindow(); // Get the GLFW window
    // Load GLAD
    ...
    Renderer renderer(projection); // Create a Renderer instance
    
    Control control(window, view); // Create a Control instance
    
    ImguiInterface imgui(window); // Create an ImguiInterface instance
    
    windowSystem.SetRenderer(renderer); // Update the window size in WindowSystem
    ...
    shader.SetUniform3f("u_LightColor", 1.0f, 1.0f, 1.0f); // Cyan light
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!fileParser.modelNames.empty()) {
            ImGui::Combo("Select Model", &selectedModel, fileParser.modelNames.data(), (int)fileParser.modelNames.size());
            
            // Controls for selected model
            ImGui::Separator();
            ImGui::Text("%.3f ms/frame (%.1f FPS)", 1000.0f / imgui.io.Framerate, imgui.io.Framerate);
            ImGui::Separator();
            ImGui::Text("Transform");
            ImGui::DragFloat3("Position", glm::value_ptr(fileParser.objVector[selectedModel].position), 0.01f);
            ImGui::DragFloat3("Rotation", glm::value_ptr(fileParser.objVector[selectedModel].rotation), 1.0f);
            ImGui::DragFloat3("Scale",    glm::value_ptr(fileParser.objVector[selectedModel].scale),    0.01f, 0.01f, 100.0f);
        }
